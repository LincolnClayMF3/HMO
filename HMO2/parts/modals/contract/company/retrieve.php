...
$result = mysqli_query($con,"SELECT * FROM tblcompanymaintenance");
  ...
  <div class="modal fade modal-warning" id="retrieveCompany<?php echo $row['companyMainID']; ?>"  role="dialog" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content" style="margin-top: 10em; margin-left: 3em">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title">Retrieve Company</h4>
        </div>
        <div class="modal-body">
          <form role="form" method="post">
          <p>Do You Want To Retrieve <?php echo $row['companyMainName'] ?> ?</p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-danger pull-left" data-dismiss="modal" name="btnCancel">Close</button>
          <input type="hidden"  name="id" value="<?php echo $row['companyMainID']?>">
...
if(isset($_POST['btnRetrieve'])){
  ...
  require 'parts/php/post/contract/company/btnRetrieve.php';

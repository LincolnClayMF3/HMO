...
if($status == "Terminated"){
            ...
            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE
                                      status = '$status' ORDER By memberTransName ASC");

            }

            else if($frequency == "Daily"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEndContract > DATE_SUB(NOW(), INTERVAL 1 DAY)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Weekly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEndContract > DATE_SUB(NOW(), INTERVAL 1 WEEK)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Monthly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEndContract > DATE_SUB(NOW(), INTERVAL 1 MONTH)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Yearly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEndContract > DATE_SUB(NOW(), INTERVAL 1 YEAR)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }
                ...
                $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEndContract BETWEEN '$fromDate' AND '$toDate'
                                      AND status = '$status' ORDER By memberTransName ASC");
            }
            ...
            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 DAY)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Weekly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 WEEK)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Monthly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 MONTH)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }

            else if($frequency == "Yearly"){

            $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 YEAR)
                                      AND status = '$status' ORDER By memberTransName ASC");
            }
                ...
                $fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction WHERE memberEffectivityDate BETWEEN '$fromDate' AND '$toDate'
                                      AND status = '$status' ORDER By memberTransName ASC");
            }
}
...
//$fetchMember = mysqli_query($con, "SELECT * FROM tblmembertransaction
			...
			if(mysqli_num_rows($fetchMember)>0)
				{
				while($row = mysqli_fetch_array($fetchMember)){
                    $contractname = $row['memberTransName'];
                    $convertDateEntry = date("M jS, Y", strtotime($row[20]));
                    if($row[21] == "Open Ended Contract"){
                        $convertDateEnd = "Open Ended Contract";
                    }
                    else{
                        $convertDateEnd = date("M jS, Y", strtotime($row[21]));
                    }

				/*$pdf->Cell(55, 10, "$row[1]",1 ,0, 'C');
		        $pdf->Cell(31, 10, $convertDateEntry, 1,0, 'C');
		        $pdf->Cell(40, 10, $convertDateEnd,1 ,0, 'C');
		        $pdf->Cell(30, 10, "Member", 1,0, 'C');
		        $pdf->Cell(30, 10, $status, 1,0, 'C');
		        $pdf->Ln(10);*/
                $pdf->SetWidths(array(55,31,40,30,30));
                $pdf->Row(array($contractname, $convertDateEntry, $convertDateEnd, 'Member', $status));
		        $total++;
		    	}
				}

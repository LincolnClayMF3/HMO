...
if($status == "Terminated"){
            ...
            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE
                                      status = '$status' ORDER By companyMainName ASC");

            }

            else if($frequency == "Daily"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEndContract > DATE_SUB(NOW(), INTERVAL 1 DAY)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Weekly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEndContract > DATE_SUB(NOW(), INTERVAL 1 WEEK)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Monthly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEndContract > DATE_SUB(NOW(), INTERVAL 1 MONTH)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Yearly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEndContract > DATE_SUB(NOW(), INTERVAL 1 YEAR)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }
                ...
                $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEndContract BETWEEN '$fromDate' AND '$toDate'
                                      AND status = '$status' ORDER By companyMainName ASC");
            }
            ...
            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 DAY)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Weekly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 WEEK)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Monthly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 MONTH)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }

            else if($frequency == "Yearly"){

            $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEffectivityDate > DATE_SUB(NOW(), INTERVAL 1 YEAR)
                                      AND status = '$status' ORDER By companyMainName ASC");
            }
                ...
                $fetchCompany = mysqli_query($con, "SELECT * FROM tblcompanymaintenance WHERE companyEffectivityDate BETWEEN '$fromDate' AND '$toDate'
                                      AND status = '$status' ORDER By companyMainName ASC");
            }
}
...
$pdf->SetFont('courier','',9);
			...
			if(mysqli_num_rows($fetchCompany)>0)
				{
				while($row = mysqli_fetch_array($fetchCompany)){
                    $contractname = $row['companyMainName'];
                    $status = $row['status'];
                    $convertDateEntry = date("M jS, Y", strtotime($row[8]));
                    if($row[9] == "Open Ended Contract"){
                        $convertDateEnd = "Open Ended Contract";
                    }
                    else{
                        $convertDateEnd = date("M jS, Y", strtotime($row[9]));
                    }
		        ...
		        $pdf->Cell(30, 10, "$row[11]", 1,0, 'C');
		        $pdf->Ln(10);*/
                $pdf->SetWidths(array(55,31,40,30,30));
                $pdf->Row(array($contractname, $convertDateEntry, $convertDateEnd, 'Company', $status));
		        $total++;
		    	}
				}
